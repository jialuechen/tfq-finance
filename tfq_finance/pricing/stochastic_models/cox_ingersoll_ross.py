...
def simulate_cox_ingersoll_ross(initial_value, long_term_mean, mean_reversion_speed, volatility, time_horizon, num_steps, num_qubits=3, steps=100, learning_rate=0.1):
    ...
    def expected_value(params):
        ...
        for _ in range(num_steps):
            ...
            dr = mean_reversion_speed * (long_term_mean - values[-1]) * dt + volatility * np.sqrt(values[-1]) * dW
            values.append(values[-1] + dr)
...
if __name__ == "__main__":
    ...
    cir_value = simulate_cox_ingersoll_ross(initial_value, long_term_mean, mean_reversion_speed, volatility, time_horizon, num_steps)
    print(f"Cox-Ingersoll-Ross Simulated Value: {cir_value}")