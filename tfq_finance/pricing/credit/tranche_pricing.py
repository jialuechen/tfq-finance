...
def price_tranche(notional, attachment_point, detachment_point, default_prob, recovery_rate, maturity, num_qubits=3, steps=100, learning_rate=0.1):
    ...
    def expected_tranche_value(params):
    ...
    for step in range(steps):
        loss = model.train_on_batch(quantum_data, expected_tranche_value(param_values))
        param_values -= learning_rate * loss

    return expected_tranche_value(param_values)
...
if __name__ == "__main__":
    ...
    tranche_value = price_tranche(notional, attachment_point, detachment_point, default_prob, recovery_rate, maturity)
    print(f"Tranche Value: {tranche_value}")