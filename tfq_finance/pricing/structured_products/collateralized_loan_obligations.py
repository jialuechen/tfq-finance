...
def price_clo(notional, attachment_point, detachment_point, default_prob, recovery_rate, maturity, num_qubits=3, steps=100, learning_rate=0.1):
    ...
    for step in range(steps):
        loss = model.train_on_batch(quantum_data, expected_clo_value(param_values))
        param_values -= learning_rate * loss

    return expected_clo_value(param_values)
...
if __name__ == "__main__":
    ...
    clo_value = price_clo(notional, attachment_point, detachment_point, default_prob, recovery_rate, maturity)
    print(f"CLO Value: {clo_value}")